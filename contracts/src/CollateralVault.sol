...
contract CollateralVault is Interest {
    ...
    struct Account {
        ...
        address owed_token;  // Address of the ERC20 token to pay out in
        uint owed_amt; // Max payout amt 
        // tab?
        ...
        uint last_accrual;    // Time of last interest update. Every time an interest
        ...
        bool useContractGlobalParams; // in case the managing contract wants to
            ...
            // and we wont check any contract globals, we'll just use zeroes
    ...
    function open(
        ...
        uint owed_amt,
        uint rate,
        uint mcr,           // Minimum collateralization ratio, as a ray
        address[] menu
    )
        public returns (bool) 
    {
        // Check that account doesn't exist
        require(accounts[msg.sender][holder].mcr == 0, "collateral-vault-open-account-exists");
        
        // Owed token cannot be 0
        require(owed_token != address(0), "collateral-vault-open-owed-token-invalid");
